...
def setup_database():
    ...
    bugs = [
        ...
        models.Bug(severity="medium", description="Medium severity bug", reported_by_id=users[2].id,
                   contest_id=contest.id),
        models.Bug(severity="medium", description="Another Medium severity bug", reported_by_id=users[3].id,
                   contest_id=contest.id),
    ]
...
def test_process_elo_with_elo_assertions(setup_database: Session):
    ...
    assert elo_histories["user1"] > elo_histories[
        "user2"], "User with critical bug should gain more ELO than high severity bug."
    assert elo_histories["user2"] > elo_histories[
        "user3"], "User with high severity bug should gain more ELO than medium severity bug."
