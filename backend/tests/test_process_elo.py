...
def setup_database():
    ...
    users = [
        ...
        models.User(username="user3", email="user3@example.com", hashed_password="hashedpassword3", role="watson"),
        models.User(username="user4", email="user4@example.com", hashed_password="hashedpassword4", role="watson"),
    ]
    ...
    bugs = [
        ...
        models.Bug(severity="high", description="High severity bug", reported_by_id=users[1].id, contest_id=contest.id),
        models.Bug(severity="medium", description="Medium severity bug", reported_by_id=users[2].id, contest_id=contest.id),
        models.Bug(severity="medium", description="Another Medium severity bug", reported_by_id=users[3].id, contest_id=contest.id),
    ]
...
def test_process_elo_with_elo_assertions(setup_database: Session):
    ...
    elo_histories = {}
    for user in users:
        elo_history = session.query(models.EloHistory).filter_by(user_id=user.id).order_by(models.EloHistory.id).all()
        assert len(elo_history) > 0, f"ELO history should be created for user {user.username}"
        elo_histories[user.username] = elo_history[-1].elo_points_after

    # Check that users with higher severity bugs gain more ELO
    assert elo_histories["user1"] > elo_histories["user2"], "User with critical bug should gain more ELO than high severity bug."
    assert elo_histories["user2"] > elo_histories["user3"], "User with high severity bug should gain more ELO than medium severity bug."
    assert elo_histories["user3"] == elo_histories["user4"], "Users with same severity bugs should gain the same ELO."
...
def test_update_user_roles_with_elo(setup_database: Session):
    ...
    session = setup_database
    senior_watson = session.query(models.User).filter(models.User.role == "senior_watson").all()
    watson = session.query(models.User).filter(models.User.role == "watson").all()

    assert len(senior_watson) == 4, "All users should be assigned a higher role after ELO processing"
    assert len(watson) == 0, "All users should be assigned a higher role after ELO processing"
...
def test_process_participation_days_contest_still_running(setup_database: Session):
    ...
    assert contest.end_date > datetime.now(timezone.utc), "Contest end_date should be in the future"
    ...
    assert response.status_code == 400, f"Expected 400, but got {response.status_code} instead"
